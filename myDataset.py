...
# using sort(): {'colorization': 0, 'nature': 1}
...
class MyDataset(data.Dataset):
    ...
    def __init__(self, args, transform=None, target_transform=None,
        ...
        classes, class_to_idx = find_classes(args.dataroot)
        imgs, num_in_class, images_txt = make_dataset(args.dataroot, class_to_idx)
        if len(imgs) == 0:
            raise(RuntimeError("Found 0 images in subfolders of: " + args.dataroot + "\n"
...
class RandomBalancedSampler(Sampler):
    ...
    def __iter__(self):
        ...
        return iter(next(it) for it in \
                    itertools.cycle([iter(a_perm), iter(b_perm)]))
...
class DataLoaderHalf(DataLoader):
    def __init__(self, dataset,
                 shuffle=False, batch_size=1, half_constraint=False, sampler_type='RandomBalancedSampler', drop_last=True, 
